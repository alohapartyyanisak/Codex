...
_CACHE_DB_PATH = Path(os.getenv("YT_RESOLVER_CACHE_PATH", "~/.cache/dj_mixing_station/youtube_resolver_cache.sqlite3")).expanduser()
_CACHE_READY = False
...
def _api_budget_cap() -> int:
    raw = os.getenv("YOUTUBE_API_RUN_BUDGET_UNITS", "").strip()
    try:
...
def _api_units_used_session() -> int:
    raw = os.getenv("YOUTUBE_API_UNITS_USED_SESSION", "0").strip()
    try:
        return max(0, int(raw))
    except Exception:
        return 0
...
def _api_units_add_session(units: int) -> None:
    ...
    os.environ["YOUTUBE_API_UNITS_USED_SESSION"] = str(used)

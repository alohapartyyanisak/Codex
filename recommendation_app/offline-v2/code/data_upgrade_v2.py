...
BASELINE_DATASET_ID = "salvatorerastelli/spotify-and-youtube"
EXPANSION_DATASET_ID = "solomonameh/spotify-music-dataset"
_DEFAULT_MERGE_CACHE_DIR = Path(__file__).resolve().parent.parent / ".cache" / "merge_cache"
_MERGE_CACHE_DIR = Path(os.getenv("DJ_MERGE_CACHE_DIR", str(_DEFAULT_MERGE_CACHE_DIR))).expanduser()
...
def _extract_youtube_video_id(url: object) -> str | None:
    ...
    watch = re.search(r"[?&]v=([A-Za-z0-9_-]{6,})", text)
    if watch:
        return watch.group(1)
    embed = re.search(r"/(?:embed|shorts)/([A-Za-z0-9_-]{6,})", text)
...
def load_and_merge_from_kagglehub(
    ...
    cache_disabled = str(os.getenv("DJ_DISABLE_MERGE_CACHE", "")).strip().lower() in {"1", "true", "yes", "y"}

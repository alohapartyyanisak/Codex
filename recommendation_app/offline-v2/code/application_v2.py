...
def render_modern_debug_table_v2(playlist: pd.DataFrame) -> None:
    ...
    for idx, row in safe_playlist.reset_index(drop=True).iterrows():
        youtube_ref = row.get("youtube_link") or row.get("url_youtube") or ""
        ...
        confidence_label = "Low (0.00)"
        if meta:
            confidence_label = f"{meta.get('confidence', 'Low')} ({meta.get('score', '0.00')})"
    ...
    base_app.st.markdown(
        (
            ...
            "<thead><tr>"
            "<th>Rank</th><th>Artist</th><th>Track</th><th>Direct YouTube</th><th>YouTube Resolve Cache</th><th>Cache Expires In</th><th>YouTube Confidence</th><th>Duration</th><th>Match</th><th>Momentum</th><th>Views</th><th>Streams</th>"
            "</tr></thead>"
